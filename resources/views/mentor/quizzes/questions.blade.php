    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form action="{{ route('mentor.quizzes.questions.add', $quiz) }}" method="POST" class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 md:col-span-2">
                <select aria-label="Type" name="type" class="w-full min-w-0 rounded-lg border-gray-200 border px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-sm font-bold text-blue-600 flex-shrink-0">
                                {{ $loop->iteration }}
                            </span>
            ...
            <div class="px-6 pb-6">
                <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                        <form action="{{ route('mentor.quizzes.answers.update', [$quiz, $question, $answer]) }}" method="POST" class="flex-1 flex items-center gap-2 sm:gap-3 min-w-0">
                            @csrf
                            @method('PATCH')
                            <input aria-label="Answer Text" type="text" name="answer_text" value="{{ $answer->answer_text }}" required
                                class="flex-1 min-w-0 rounded-lg border border-gray-200 px-2 sm:px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <!-- Add Answer -->
                    <form action="{{ route('mentor.quizzes.answers.add', [$quiz, $question]) }}" method="POST" class="flex items-center gap-2 pt-2 border-t border-gray-200 min-w-0">
                        @csrf
