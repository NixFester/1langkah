...
@section('title', 'Sesi Mentoring Saya - 1Langkah')
@section('header_title', 'Sesi Mentoring Saya')
...
<div class="w-full px-2 pb-12 space-y-6">
        ...
        <div class="flex items-start justify-between mb-4">
            <div>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                    ...
                    {{ $activeSession->isPending() ? 'Menunggu Konfirmasi' : 'Aktif' }}
                </span>
                <h2 class="text-xl font-bold text-gray-900 mt-3">Sesi Mentoring dengan {{ $activeSession->mentor->name }}</h2>
                <p class="text-gray-600 mt-1">{{ $activeSession->formatted_date }} • Pukul {{ $activeSession->booked_time }}</p>
            </div>
        </div>
        ...
        @if($activeSession->isPending())
            <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-sm text-yellow-800">Mentor akan menghubungi Anda melalui WhatsApp dalam 2x24 jam.</p>
                </div>
            </div>
            <form action="{{ route('session.cancel', $activeSession) }}" method="POST" class="inline">
                @csrf @method('PATCH')
                <button type="submit" class="px-4 py-2 bg-white border border-gray-200 text-gray-700 font-medium rounded-full hover:bg-gray-50 transition-colors text-sm" onclick="return confirm('Batalkan sesi mentoring ini?')">
                    Batalkan Booking
                </button>
            </form>
        @else
            <div class="bg-white border border-gray-100 rounded-xl p-4 mb-4">
                <p class="text-sm text-gray-600 mb-3">Silakan hubungi mentor untuk memulai sesi mentoring:</p>
            ...
            <p class="text-xs text-gray-500">Sesi mentoring dianggap selesai ketika mentor menekan tombol "Selesai"</p>
        @endif
    </div>
    @endif
    ...
    <div class="bg-white border border-gray-100 rounded-3xl p-8 text-center">
        ...
        <h2 class="text-xl font-bold text-gray-900 mb-2">Belum Ada Sesi Mentoring Aktif</h2>
        <p class="text-gray-600 mb-6">Pesan sesi mentoring dengan mentor pilihan Anda untuk memulai perjalanan belajar!</p>
    ...
    <div class="bg-white border border-gray-100 rounded-3xl p-6 md:p-8">
        <h3 class="text-lg font-bold text-gray-900 mb-5">Riwayat Sesi Mentoring</h3>
        <div class="space-y-4">
            ...
            <div class="border border-gray-100 rounded-xl p-4 {{ $session->isCompleted() ? '' : 'opacity-60' }}">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="px-3 py-1 rounded-full text-xs font-bold
                        ...
                        {{ $session->isCompleted() ? 'Selesai' : 'Dibatalkan' }}
                    </span>
                </div>
