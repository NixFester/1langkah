...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <x-stat-card :label="__('app.total_users')" :value="$userStats['total']" color="purple" />
        <x-stat-card :label="__('app.admin')" :value="$userStats['admins']" color="red" />
        <x-stat-card :label="__('app.finance')" :value="$userStats['keuangans']" color="amber" />
        <x-stat-card :label="__('app.marketing')" :value="$userStats['marketings']" color="pink" />
        <x-stat-card :label="__('app.mentor')" :value="$userStats['mentors']" color="blue" />
    </div>

    {{-- System Stats --}}
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <x-gradient-stat :label="__('app.course')" :value="$systemStats['courses']" from="from-purple-500" to="to-purple-600" />
        <x-gradient-stat :label="__('app.enrollments')" :value="$systemStats['enrollments']" from="from-blue-500" to="to-blue-600" />
        <x-gradient-stat :label="__('app.revenue_this_month')" :value="'Rp ' . number_format($systemStats['revenue_this_month'], 0, ',', '.')" from="from-green-500" to="to-green-600" />
        <x-gradient-stat :label="__('app.pending_payment')" :value="$systemStats['pending_payments']" from="from-amber-500" to="to-amber-600" />
    </div>
    ...
    <x-card-panel :title="__('app.quick_actions')">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <a href="{{ route('superadmin.users') }}" class="p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors text-center">
                ...
                <p class="text-sm font-medium text-purple-800">{{ __('app.manage_users') }}</p>
            </a>
            <a href="{{ route('superadmin.audit-logs') }}" class="p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors text-center">
                ...
                <p class="text-sm font-medium text-blue-800">{{ __('app.audit_log') }}</p>
            </a>
            <a href="{{ route('superadmin.system-stats') }}" class="p-4 bg-green-50 rounded-lg hover:bg-green-100 transition-colors text-center">
                ...
                <p class="text-sm font-medium text-green-800">{{ __('app.statistics') }}</p>
            </a>
            <a href="{{ route('dashboard') }}" target="_blank" class="p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-center">
                ...
                <p class="text-sm font-medium text-gray-800">{{ __('app.open_app') }}</p>
            </a>
        </div>
    </x-card-panel>
@endsection
