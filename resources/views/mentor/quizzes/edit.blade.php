    ...
    <x-form-card>
        <form action="{{ isset($quiz) ? route('mentor.quizzes.update', $quiz) : route('mentor.quizzes.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', isset($quiz) ? $quiz->title : '') }}" required class="w-full rounded-lg border-gray-200 border px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="{{ __('app.example_quiz_title') }}">
                @error('title')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
